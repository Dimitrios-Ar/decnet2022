from penet2021.penet2021_basic import *
...
class ENet(nn.Module):
    def __init__(self, args):
        ...
        self.rgb_encoder_layer1 = BasicBlockGeo(inplanes=int(32/simplification_factor), planes=int(64/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.rgb_encoder_layer2 = BasicBlockGeo(inplanes=int(64/simplification_factor), planes=int(64/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.rgb_encoder_layer3 = BasicBlockGeo(inplanes=int(64/simplification_factor), planes=int(128/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.rgb_encoder_layer4 = BasicBlockGeo(inplanes=int(128/simplification_factor), planes=int(128/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.rgb_encoder_layer5 = BasicBlockGeo(inplanes=int(128/simplification_factor), planes=int(256/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.rgb_encoder_layer6 = BasicBlockGeo(inplanes=int(256/simplification_factor), planes=int(256/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.rgb_encoder_layer7 = BasicBlockGeo(inplanes=int(256/simplification_factor), planes=int(512/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.rgb_encoder_layer8 = BasicBlockGeo(inplanes=int(512/simplification_factor), planes=int(512/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.rgb_encoder_layer9 = BasicBlockGeo(inplanes=int(512/simplification_factor), planes=int(1024/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.rgb_encoder_layer10 = BasicBlockGeo(inplanes=int(1024/simplification_factor), planes=int(1024/simplification_factor), stride=1, geoplanes=self.geoplanes)

        self.rgb_decoder_layer8 = deconvbnrelu(in_channels=int(1024/simplification_factor), out_channels=int(512/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.rgb_decoder_layer6 = deconvbnrelu(in_channels=int(512/simplification_factor), out_channels=int(256/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.rgb_decoder_layer4 = deconvbnrelu(in_channels=int(256/simplification_factor), out_channels=int(128/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.rgb_decoder_layer2 = deconvbnrelu(in_channels=int(128/simplification_factor), out_channels=int(64/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.rgb_decoder_layer0 = deconvbnrelu(in_channels=int(64/simplification_factor), out_channels=int(32/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.rgb_decoder_output = deconvbnrelu(in_channels=int(32/simplification_factor), out_channels=2, kernel_size=3, stride=1, padding=1, output_padding=0)


        # depth encoder
        self.depth_conv_init = convbnrelu(in_channels=2, out_channels=int(32/simplification_factor), kernel_size=5, stride=1, padding=2)

        self.depth_layer1 = BasicBlockGeo(inplanes=int(32/simplification_factor), planes=int(64/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.depth_layer2 = BasicBlockGeo(inplanes=int(64/simplification_factor), planes=int(64/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.depth_layer3 = BasicBlockGeo(inplanes=int(128/simplification_factor), planes=int(128/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.depth_layer4 = BasicBlockGeo(inplanes=int(128/simplification_factor), planes=int(128/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.depth_layer5 = BasicBlockGeo(inplanes=int(256/simplification_factor), planes=int(256/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.depth_layer6 = BasicBlockGeo(inplanes=int(256/simplification_factor), planes=int(256/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.depth_layer7 = BasicBlockGeo(inplanes=int(512/simplification_factor), planes=int(512/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.depth_layer8 = BasicBlockGeo(inplanes=int(512/simplification_factor), planes=int(512/simplification_factor), stride=1, geoplanes=self.geoplanes)
        self.depth_layer9 = BasicBlockGeo(inplanes=int(1024/simplification_factor), planes=int(1024/simplification_factor), stride=2, geoplanes=self.geoplanes)
        self.depth_layer10 = BasicBlockGeo(inplanes=int(1024/simplification_factor), planes=int(1024/simplification_factor), stride=1, geoplanes=self.geoplanes)

        # decoder
        self.decoder_layer1 = deconvbnrelu(in_channels=int(1024/simplification_factor), out_channels=int(512/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.decoder_layer2 = deconvbnrelu(in_channels=int(512/simplification_factor), out_channels=int(256/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.decoder_layer3 = deconvbnrelu(in_channels=int(256/simplification_factor), out_channels=int(128/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.decoder_layer4 = deconvbnrelu(in_channels=int(128/simplification_factor), out_channels=int(64/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)
        self.decoder_layer5 = deconvbnrelu(in_channels=int(64/simplification_factor), out_channels=int(32/simplification_factor), kernel_size=5, stride=2, padding=2, output_padding=1)

        self.decoder_layer6 = convbnrelu(in_channels=int(32/simplification_factor), out_channels=2, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, input):
        ...
        if self.args.convolutional_layer_encoding == "xyz":
            geo_s1 = self.geofeature(d, vnorm, unorm, width, height, c352, c1216, f352, f1216)
            geo_s2 = self.geofeature(d_s2, vnorm_s2, unorm_s2, width / 2, height / 2, c352, c1216, f352, f1216)
            geo_s3 = self.geofeature(d_s3, vnorm_s3, unorm_s3, width / 4, height / 4, c352, c1216, f352, f1216)
            geo_s4 = self.geofeature(d_s4, vnorm_s4, unorm_s4, width / 8, height / 8, c352, c1216, f352, f1216)
            geo_s5 = self.geofeature(d_s5, vnorm_s5, unorm_s5, width / 16, height / 16, c352, c1216, f352, f1216)
            geo_s6 = self.geofeature(d_s6, vnorm_s6, unorm_s6, width / 32, height / 32, c352, c1216, f352, f1216)
        elif self.args.convolutional_layer_encoding == "uv":
        ...
        #print("rgb_feature: %s" % str(rgb_feature.shape))
        #print("geo_s1: %s" % str(geo_s1.shape))
        #print("geo_s2: %s" % str(geo_s2.shape))
